...
* In-scope (strategic): migration to `terraform-plugin-framework`. This fork has started an incremental migration using `terraform-plugin-mux` (Framework serves data sources first; resources migrate over time).
...
* Added global rate limit coordination in the REST client (global 429 cooldown shared across concurrent requests).
* Added opt-in acceptance tests (`-tags=acctest`) and a PowerShell helper script (`scripts/testacc.ps1`).
* Started Terraform-side migration to `terraform-plugin-framework` using `terraform-plugin-mux`:
  * Framework provider serves all data sources.
  * SDK provider continues serving resources during migration.
