...
* In-scope (strategic): migration to `terraform-plugin-framework`. This fork migrated to a Framework-only provider implementation (protocol v6).
...
* Migrated to a Framework-only provider (protocol v6) and removed the legacy SDK + mux plumbing.
